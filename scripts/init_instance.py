...
Начни с руководства агента `agent_guide.md` в локальном клоне
md-реализации (по умолчанию `../kimbi-ms-md/agent_guide.md`;
см. указатель): начальная загрузка, сборка контекста, сценарии работы.
...
Знания системы живут в своих репозиториях:

- **kimbi-ms-framework** — абстрактный фреймворк: понятия, статусы, связи.
  {FRAMEWORK_URL}
- **kimbi-ms-md** — реализация на markdown и git: форматы, процедуры,
  шаблоны, скрипты, руководство агента.
  {MD_URL}

Локальные клоны — рядом с директорией инстанса:
`../kimbi-ms-framework`, `../kimbi-ms-md`.
"""

STATUS_DIRS = ("1 draft", "2 in_progress", "3 ended")
